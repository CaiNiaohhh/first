...
def cmp_BEC_run_time(cnt, N, init_value, L):
    ...
    # print("N =", N, "K =", K, " 的情况下 SC译码所用时间为: ", t1)
    ...
    # print("N =", N, "K =", K, "L =", L, " 的情况下 SCL译码所用时间为: ", t2)
    return t1, t2
...
def cmp_BEC_correct(cnt, N, init_value, L):
    ...
    c1 = c1 / (K * cnt)
    c2 = c2 / (K * cnt)
    print("N =", N, "K =", K, " 的情况下运行 ",cnt, "次,SC译码准确率为: ", c1)
    print("N =", N, "K =", K, "L =", L, " 的情况下运行 ",cnt, "次,SCL译码准确率为: ", c2)
    return c1, c2
...
def cmp_AWGN_run_time(cnt, N, init_value, SNR, L):
    ...
    K = int(N * init_value)
    valid_msg = [1] * K
    valid_index_list, _, _, y_msg = BAWGNC_Encode.encode(valid_msg, N, init_value, SNR)
    # 计算SC运行所花费的时间
    start_time = timer()
    for _ in range(cnt):
        u_msg = np.zeros(N, dtype='uint8')  # 保存计算结果
        AWGN_Decode.Polar_Decode(valid_index_list, N, y_msg)
    end_time = timer()
    t1 = (end_time - start_time) / cnt
    # print("N =", N, "K =", K, " 的情况下 SC译码所用时间为: ", t1)
    ...
    start_time = timer()
    for _ in range(cnt):
        AWGN_Decode.AWGN_SCL_Decode(L, valid_index_list, N, y_msg)
    end_time = timer()
    t2 = (end_time - start_time) / cnt
    # print("N =", N, "K =", K, "L =", L, " 的情况下 SCL译码所用时间为: ", t2)
    return t1, t2
...
# L不变，验证N=N_list的情况下所得到的图像
def draw_BEC_run_time_N(N_list, cnt, init_value, L):
    # 保存数据
    t1_list, t2_list = [], []
    for N in N_list:
    ...
    x_name = "N"
    y_name = "run time"
    title = "BEC && SC && SCL L = " + str(L)
    label1 = "SC"
    label2 = "SCL && L = " + str(L)
    draw_picture(N_list, t1_list, t2_list, x_name, y_name, title, label1, label2)

# N不变，验证L=L_list的情况下所得到的图像
def draw_BEC_run_time_L(N, cnt, init_value, L_list):
    # 保存数据
    t1_list, t2_list = [], []
    for L in L_list:
        t1, t2 = cmp_BEC_run_time(cnt, N, init_value, L)
    ...
    x_name = "L"
    y_name = "run time"
    title = "BEC && SC && SCL && N = " + str(N)
    label1 = "SC"
    label2 = "SCL"
    draw_picture(L_list, t1_list, t2_list, x_name, y_name, title, label1, label2)

# L不变，验证N=N_list的情况下所得到的图像
def draw_BEC_correct_N(N_list, cnt, init_value, L):
    # 保存数据
    c1_list, c2_list = [], []
    for N in N_list:
...
# N不变，验证L=L_list的情况下所得到的图像
def draw_BEC_correct_L(N, cnt, init_value, L_list):
    c1 = 0
...
if __name__ == "__main__":
    ...
    N_list = [2**i for i in range(4, 8)]
    L_list = [2*i for i in range(2, 6)]
    # draw_BEC_run_time_N(N_list, cnt, init_value, L)
    # draw_BEC_run_time_L(N, cnt, init_value, L_list)
    # draw_BEC_correct_N(N_list, cnt, init_value, L)
    draw_BEC_correct_L(N, cnt, init_value, L_list)
